...
function createWindow(): void {
  ...
  const mainWindow = new BrowserWindow({
    ...
    icon: process.platform === 'win32' 
      ? path.join(__dirname, '../../resources/icon.ico')  // Для Windows
      : path.join(__dirname, '../../resources/icon.png'), // Для Linux
    webPreferences: {
...
// ipcMain.on('change-brightness', async () => {
...
// mainWindow.webContents.send('monitors-data', monitors)

async function getMonitorBrightness() {
  let monitor_brightness: { id: string; name: string; brightness: number }[] = [];

  for (let i = 0; i < monitors.length; i++) {
    const monitorId = monitors[i]['id']; 
    const {success: _, brightness} = await lumi.get(monitorId);
    // monitor_brightness[monitors[i]['name']] = brightness; 
...
ipcMain.on('update-brightness', (_event, {id, value}) => {
  lumi.set(id, value)
});